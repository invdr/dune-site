...
- TanStack Form
- TanStack Router
- Zod contracts from `@dune/contracts`
- shadcn CLI
- Playwright
- ESLint
...
`VITE_API_URL` is build-time config. In production it must be a concrete backend origin such as `https://api.example.com`; if it changes, rebuild and redeploy the static bundle so it stops using the old URL.

## Deployment

Production builds the browser app to static files (`bun run build:webapp` → `webapp/dist`) and serves them from Caddy on the VPS, with `index.html` as the SPA catch-all for client-side routing. The build happens inside `deploy/Dockerfile.web` from the full Git monorepo checkout, with `VITE_API_URL` injected at build time. Follow the shared runbook in [../docs/DEPLOYMENT.md](../docs/DEPLOYMENT.md).
